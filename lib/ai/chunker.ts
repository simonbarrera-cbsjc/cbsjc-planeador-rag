 ...
 * @returns Array of chunk strings, each at least 50 characters long.
 */
export function chunkText(
  ...
  const overlapChars = overlap * 4;   // tokens -> chars
    ...
    for (const sentence of sentences) {
      if ((buffer + ' ' + sentence).length <= chunkChars) {
        ...
        for (let i = 0; i < hardChunks.length - 1; i++) {
          if (hardChunks[i].length >= 50) chunks.push(hardChunks[i]);
        }
...
function hardSplit(
  ...
  while (start < text.length) {
    ...
    start += Math.max(maxChars - overlapChars, 1);
  }
